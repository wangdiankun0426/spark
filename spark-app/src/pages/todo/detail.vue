  ...
  <view class="detail-container safe-area-page">
    ...
    <view class="detail-footer" v-if="showFooter">
      ...
      <template v-else-if="type === 1">
        <up-button type="warning" size="small" text="催办" :loading="urging" @click="handleUrge"/>
      </template>
    </view>
    ...
    <user-picker
        ...
        :title="operateType === 'transfer' ? '选择转办人' : '选择加签人'"
...
const showFooter = computed(() => {
  if (type.value === 3) {
    return nodeId.value !== undefined && (hasPermission(1) || hasPermission(2) || hasPermission(16) || hasPermission(32));
  }
  if (type.value === 1) {
    // 仅审批中的流程可催办
    return nodeId.value !== undefined && instance.value.status === 2 && hasPermission(8);
  }
  return false;
});
...
function handleUserConfirm(users) {
  ...
  api({
    id: instanceId.value,
    nodeId: nodeId.value,
    assigneeIds: users.map(user => user.id).join(',')
  }).then(res => {
    if (res.code === 200) {
