<template>
  <view class="todo-container safe-area-page">
    <!-- 顶部标题栏 -->
    ...
    <!-- 抄送给我（开发中） -->
    <view v-if="tabIndex === 3" class="empty-box">
      <up-empty text="功能开发中" icon="list"/>
    </view>

    <!-- 流程实例列表 -->
    <scroll-view
        v-else class="todo-list"
        scroll-y
    ...
    >
      ...
      <!-- 实例卡片 -->
      <template v-else>
        <view class="instance-card" v-for="item in instanceList" :key="item.id" @click="handleInstanceClick(item)">
...
import {
  pageMyAppliedListAPI,
  pageMyPendingListAPI,
  pageMyPendedListAPI
...
const total = ref(0);
// 页签对应的列表接口：1 我的申请 3 我的待办 4 我的已办
const tabApiList = [pageMyAppliedListAPI, pageMyPendingListAPI, pageMyPendedListAPI];
// 页签对应的详情类型（详情页按类型展示操作按钮）
const tabTypeList = [1, 3, 4];
...
function handleLoadMore() {
  if (tabIndex.value === 3 || loading.value || loadStatus.value === 'nomore') {
    return;
  }
...
function handleInstanceClick(item) {
  uni.navigateTo({
    url: '/pages/todo/detail?id=' + item.id + '&type=' + tabTypeList[tabIndex.value]
  });
}
