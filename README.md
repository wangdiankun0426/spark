...
| 🧩 **AI 工作流（Workflow）** | 可视化 **DAG 工作流设计器**：拖拽 LLM、知识检索等节点编排 AI 能力链，发布为 HTTP API 端点，可调度、可追踪 |
...
可视化 **DAG 工作流设计器**：拖拽 LLM 对话、知识库检索等节点，连线编排 AI 能力链；支持版本管理、开启/关闭、运行实例与节点级执行记录；发布为 HTTP API 端点，供外部系统调用，可调度、可追踪。
