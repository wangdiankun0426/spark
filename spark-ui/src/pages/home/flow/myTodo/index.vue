<template>
  <div class="app-container">
    ...
    <!-- 转办/加签/抄送选人弹窗 -->
    <el-dialog
        :title="dialogType === 'transfer' ? '转办' : dialogType === 'addSign' ? '加签' : '抄送'"
        v-model="operateDialogVisible"
        width="400px"
        :close-on-click-modal="false"
    >
      <el-form label-width="70px">
        <el-form-item :label="dialogType === 'transfer' ? '转办人' : dialogType === 'addSign' ? '加签人' : '抄送人'">
          <select-user
              v-model="selectedUserIds"
              :multiple="dialogType !== 'transfer'"
              clearable
              style="width: 100%"
          />
        </el-form-item>
...
import {
  ...
  pageMyTodoListAPI, showInstanceDetailAPI,
...
function handleGetInstanceList() {
  pageMyTodoListAPI(instanceQuery.value).then(res => {
...
function confirmOperate() {
  ...
  if (!ids.length) {
    const tipMap = { transfer: '请选择转办人', addSign: '请选择加签人', copy: '请选择抄送人' };
    ElMessage.warning(tipMap[dialogType.value] || '请选择人员');
    return;
  }
