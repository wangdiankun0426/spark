  ...
  <div class="app-container">
    ...
    >
      ...
      >
        ...
        <!-- 配置标签 + 连接信息 -->
        <template #tags>
          <el-tag size="small" effect="light" round v-if="item.transportName">{{ item.transportName }}</el-tag>
          <el-tag size="small" effect="light" round v-if="item.providerName">{{ item.providerName }}</el-tag>
          <el-tag size="small" effect="light" round>超时 {{ item.timeout }}s</el-tag>
          <div class="mcp-connect" v-if="item.command || item.url">
            <div class="mcp-connect-item" v-if="item.command" :title="'命令：' + item.command">
              <span class="mcp-connect-label">命令</span>
              <span class="mcp-connect-value">{{ item.command }}</span>
            </div>
            <div class="mcp-connect-item" v-else-if="item.url" :title="'URL：' + item.url">
              <span class="mcp-connect-label">URL</span>
              <span class="mcp-connect-value">{{ item.url }}</span>
            </div>
          </div>
        </template>
...
import { Connection, Search, Plus, Delete, Loading } from '@element-plus/icons-vue'
  ...
  :deep(.info-card-action) {
    ...
    .action-test {
      ...
      &.is-loading {
        cursor: default;
        .el-icon {
          animation: mcp-rotate 1s linear infinite;
        }
      }
    }
    .action-edit {
