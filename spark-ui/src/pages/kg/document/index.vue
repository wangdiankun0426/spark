  ...
  <div class="app-container">
    ...
    <div class="kd-header">
      <div class="kd-header-left">
        ...
        <div class="kd-header-title">
          <el-icon class="kd-header-icon"><Connection /></el-icon>
          {{ graphName || '知识图谱文档' }}
        </div>
      </div>
    ...
    <div style="width: 100%;">
      ...
      >
        ...
        <el-table-column fixed="right" label="操作" width="420" align="center">
          <template #default="scope">
            <el-button type="primary" text @click="handleOpenGraphDetail(scope.row.id)">
              <el-icon><Connection /></el-icon>
              <span style="font-size: 12px; font-weight: 500">图谱</span>
            </el-button>
    ...
    >
      ...
      >
        ...
        <el-form-item label="所属知识图谱" prop="kbName">
          <el-input v-model="documentForm.kbName" placeholder="所属知识图谱" readonly />
        </el-form-item>
    ...
    <!-- 文件上传 -->
    <document-upload v-model="uploadDocumentFormVisible" :prt-id="graphId" @success="handleGetDocumentList" />
...
<script setup name="graphDocument">
...
import {
  ArrowLeft, Connection, Search, Refresh, DocumentAdd,
...
// 路由参数变化时重新加载
watch(graphId, () => {
  documentQuery.value.prtId = graphId.value
  loadGraphDetail()
 ...
 * 加载知识图谱详情，用于头部标题展示
 */
function loadGraphDetail() {
  if (!graphId.value) {
    graphName.value = ''
    return
  }
  queryGraphDetailAPI({ id: graphId.value }).then(res => {
